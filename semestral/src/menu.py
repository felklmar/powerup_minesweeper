...
class Menu:
    ...
    def __init__( self, window : pg.Surface,
                  offset : tuple, colors : dict, def_diff : int ):
        """
        ...
        y_off = offset[0] + LOGO.get_height() + 20
        x_off = 2*offset[1]
        self.m_buttons = [
    ...
    def __settings_display( self ):
        ...
        y_off = self.m_offset[0] + LOGO.get_height() + 20
        x_off = 2*self.m_offset[1]
        # height: its value
        self.__display_text( 'height:', ( 200 + y_off, 60 + x_off ), self.m_colors['t_disabled'] )
        self.__display_text( str( self.m_game_settings[0] ),
                             ( 200 + y_off, 225 + x_off ), self.m_colors['t_basic'] )

        # width: its value
        self.__display_text( 'width:', ( 250 + y_off, 60 + x_off ), self.m_colors['t_disabled'] )
        self.__display_text( str( self.m_game_settings[1] ),
                             ( 250 + y_off, 225 + x_off ), self.m_colors['t_basic'] )

        # mines: its value
        self.__display_text( 'mines:', ( 200 + y_off, 420 + x_off ), self.m_colors['t_disabled'] )
        self.__display_text( str( self.m_game_settings[2] ),
                             ( 200 + y_off, 595 + x_off ), self.m_colors['t_basic'] )

        # tokens: its value
        self.__display_text( 'tokens:', ( 250 + y_off, 420 + x_off ), self.m_colors['t_disabled'] )
        self.__display_text( str( self.m_game_settings[3] ),
                             ( 250 + y_off, 595 + x_off ), self.m_colors['t_basic'] )

    def __increase( self, event : pg.event, set_idx : int, maximum : tuple ):
        """
        ...
        Args:
            ...
            maximum (tuple): width and height maximum
        """
        height = self.height()
        width = self.width()
        mines = self.mines()
    ...
    def __decrease( self, event : pg.event, set_idx : int, minimum : tuple ):
        """
        ...
        Args:
            ...
            minimum (tuple): dim, mines and tokens minimum
        """
        if event.button == 1:
            self.m_game_settings[set_idx] -= 1
        if event.button == 2:
            self.m_game_settings[set_idx] = minimum[set_idx]
        if event.button == 3:
            self.m_game_settings[set_idx] -= 5

        if self.m_game_settings[set_idx] < minimum[set_idx]:
            self.m_game_settings[set_idx] = minimum[set_idx]
    ...
    def __customization_buttons( self, event : pg.event, btn : str ):
        ...
        if btn == 'h_>':
            self.__increase( event, 0, maximum )

        if btn == 'h_<':
            self.__decrease( event, 0, minimum )

        if btn == 'w_>':
            self.__increase( event, 1, maximum )

        if btn == 'w_<':
            self.__decrease( event, 1, minimum )

        if btn == 'm_>':
            self.__increase( event, 2, maximum )

        if btn == 'm_<':
            self.__decrease( event, 2, minimum )

        if btn == 't_>':
            self.__increase( event, 3, maximum )

        if btn == 't_<':
            self.__decrease( event, 3, minimum )

    def display( self ):
        """Displays all the text and buttons on the pygame window"""
        x_off, y_off = self.m_offset[1], self.m_offset[0]
        ...
        diffc = self.m_font.render( DIFFIC[self.m_d_idx][0], True, self.m_colors['t_basic'] )
        ...
        for button in self.m_buttons:
            button.display( self.m_window, self.m_colors['background'] )
